...
定义好管道之后我们需要配置到爬虫上，我们`settings.py`模块中配置，后面的数字表示管道的顺序
...
* `link_extractor`
  ：链接提取规则
* `callback`
  ：link\_extractor提取的链接的请求结果的回调
* `cb_kwargs`
  ：附加参数，可以在回调函数中获取到
* `follow`
  ：表示提取的链接请求完成后是否还要应用当前规则（boolean），如果为
  `False`
...
在`settings.py`设置USER\_AGENTS参数
...
> 更多参数参见
>
> [这里](https://doc.scrapy.org/en/latest/topics/settings.html#concurrent-items)
...
有时候我们已经把urls下载到数据库了，而不是在start\_urls里配置，这时候可以重载spider的`start_requests`方法
...
> DUPEFILTER\_CLASS='tutorial.custom\_filters.CustomURLFilter'
